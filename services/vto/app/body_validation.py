...
def _basic_brightness_check(cv_img: np.ndarray) -> list[str]:
    issues: list[str] = []
...
def validate_body_capture(img: Image.Image) -> dict:
    ...
    issues.extend(_basic_brightness_check(cv_img))

    try:
        import mediapipe as mp
    except Exception:
        # Kaggle/cloud: mediapipe may fail due to protobuf/tensorflow conflicts
        if issues:
    ...
    try:
        ...
        with mp_pose.Pose(
            ...
            if missing:
                issues.append(
                    f"Body parts not visible: {', '.join(missing)}. Step back so your full body is in frame."
                )
