"""Light post-processing — minimal when preserving the original scene."""
...
def _clean_background_fringe(img: Image.Image) -> Image.Image:
    """Only for white-studio outputs — destroys real-world backgrounds if misapplied."""
    arr = np.array(img.convert("RGB"), dtype=np.float32)
...
def enhance_tryon_result(img: Image.Image, *, preserve_scene: bool = False) -> Image.Image:
    """Sharpen output; skip destructive passes when keeping the user's original photo."""
    out = img.convert("RGB")
    if preserve_scene:
        out = ImageEnhance.Sharpness(out).enhance(1.08)
        out = out.filter(ImageFilter.UnsharpMask(radius=0.8, percent=45, threshold=4))
        return out
